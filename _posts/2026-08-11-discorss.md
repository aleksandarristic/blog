...
date: 2026-08-11 17:27:55 +0200
tags: [python, discord, telegram, rss, self-hosted, tools]
---

DiscoRSS is my self-hosted answer to the part of RSS that still costs attention: not getting a feed, but noticing new items without babysitting a reader. It started as a Discord bot and grew into a small delivery system for Discord, Telegram, webhooks, and a local CLI.
...
DiscoRSS is self-hosted, Python, with `uv`, deployed with Docker. Discord and Telegram are both live integrations today; webhooks and the CLI reader round out the sink list. The architecture doc in the repo explicitly leaves room for more integrations — Slack gets mentioned by name as the obvious next one — without forcing them into Discord-shaped assumptions the way the first prototype would have.
